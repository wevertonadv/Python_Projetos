...
log.info("Inicando ler_coluna_excel")
...
for product_name in tqdm(list_product_name):
    ...
    # # Acessando a URL
    log.info(f"Acessando a URL {url_search_page}")
    driver.get(url_search_page)
...
for json_content in tqdm(list_json_search_page):
    ...
    log.info("Iniciando busca produtos")
...
for _, row in tqdm(df_page_product_info.iterrows(), total=df_page_product_info.shape[0]):
    ...
    log.info(f"Iniciando url produto {url_product_page}")
    ...
    try:
        botao = driver.find_element("xpath","//*[contains(text(), 'mais formas de pagamento')]")
...
print(pd.DataFrame(list_product_details))
